...
void _CascadeClassifier::LoadBinCascade()
{	
	ifstream fout("cascade.bin",ios::binary);
		...
		for(int i=0,size_x=image.height-sx;i<size_x;i+=1)
			for(int j=0,size_y=image.width-sy;j<size_y;j+=1)
			{
				...
				if(result!=0) 
				{
					...
					rect.x = (int)(j*r);rect.y = (int)(i*r);
					rect.width = (int)((j+sy)*r) - rect.x;
					rect.height = (int)((i+sx)*r) - rect.y;
